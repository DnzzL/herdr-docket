---
name: fleet-tasks
description: Create and manage tasks in the herdr-docket queue — the shared task list that herdr-docket routes to named agents. Use when asked to delegate work to a fleet agent, queue work for later, or report on fleet tasks.
---
...
Everything goes through the `herdr-docket` CLI — you do not need to know, or
care, where the queue physically lives:

```bash
herdr-docket task list                       # open work, grouped by phase
herdr-docket task list --all                 # include closed work
herdr-docket task view TASK-12               # description, criteria, notes
herdr-docket task create "Draft the launch post" \
  -d "What needs doing and why" -a dishnow-marketing
herdr-docket task assign TASK-12 dev          # hand it on, same task, same thread
herdr-docket task note TASK-12 "halfway; blocked on the API key"
herdr-docket task done TASK-12 --note "shipped; criterion 1 met, 2 dropped"
herdr-docket task fail TASK-12 --note "could not reproduce the crash"
herdr-docket task block TASK-12 --note "needs a decision on scope"
```

Rules:

- **Know which queue you are in.** A fleet may keep a queue of its own, or it
  may work a project's `backlog/` directly. Run `herdr-docket task list`: if the
  project's tickets are in it, there is one queue and nothing to separate. If
  they are not, the herdr-docket queue routes work to agents and the project's backlog
...
- **Assignee = agent.** `-a <name>` must match a folder in `<fleet>/agents/`.
  ...
  `herdr-docket list` if work seems stuck.
