...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 the phase ordering table's three closed entries are derived from the verdict vocabulary, so they cannot drift
- [x] #2 the pane styles phases from the core's words, not four hand-typed strings
- [x] #3 both adapters map their backend's labels into the core's words
- [x] #4 the board renders the same headings and colours as before: no behaviour change
