---
id: TASK-1
title: Add per-agent disabled pause to herdr-fleet
status: Done
assignee: []
...
- herdr-fleet run <id> is explicit human intent: it BYPASSES Disabled, so pause parks the scheduler rather than forbidding work.
...
- [ ] #4 CLI gains 'herdr-fleet agent list|pause|resume <name>' with usage text that states the one-tick (~15s) delay before a pause is picked up
- [ ] #5 a run already in flight is untouched by pause/resume; it finishes its own timeout and still reconciles its task
- [ ] #6 'herdr-fleet run <task-id>' starts a task whose agent is disabled, and a test locks that behavior in
